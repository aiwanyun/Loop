...
extension CarbEffectChart {
    ...
    public func setInsulinCounteractionEffects(_ effects: [GlucoseEffectVelocity]) {
        let unit = glucoseUnit.unitDivided(by: .minute())
        let unitString = String(format: NSLocalizedString("%1$@/分钟", comment: "Format string describing glucose units per minute (1: glucose unit string)"), glucoseUnit.localizedShortUnitString)
