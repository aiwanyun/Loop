...
public final class LoopCompletionHUDView: BaseHUDView {
    ...
    @objc private func updateDisplay(_: Timer?) {
        ...
        if let date = lastLoopCompleted {
            ...
            caption?.text = "–"
            accessibilityLabel = LocalizedString("等待第一次跑步", comment: "Accessibility label describing completion HUD waiting for first run")
        }

        if loopIconClosed {
            accessibilityHint = LocalizedString("闭环", comment: "Accessibility hint describing completion HUD for a closed loop")
        } else {
            accessibilityHint = LocalizedString("开环", comment: "Accessbility hint describing completion HUD for an open loop")
        }
    }
